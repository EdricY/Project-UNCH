...
def draw():
	...
	for i in range(4):
		...
		method.bufferxy(11,5+3*i,str(HEROES[i+HERO_SCREEN*4][1])) # hero level
		method.bufferxy(1,6+3*i,"&BX" + str(i+HERO_SCREEN*4) +"&XX:  " + method.dispBigNum(HEROES[i+HERO_SCREEN*4][3])) # hero cost
	if quitMenuOpen:
