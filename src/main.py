...
def draw():
	...
	method.bufferxy(5,1,method.dispBigNum(MONEY)) #money
	method.bufferxy(34,3,MOBS[CURRENT_MOB][0]) #mob name
	if CURRENT_ZONE-1>0:
		method.bufferxy(36,1,str(CURRENT_ZONE-1)) #zone num -
	if CURRENT_ZONE != HIGHEST_ZONE:
		method.bufferxy(50-len(str(CURRENT_ZONE)),1,str(CURRENT_ZONE+1)) #zone num + 
	method.bufferxy(43-len(str(CURRENT_ZONE))/2,1,str(CURRENT_ZONE)) #zone num
	if CURRENT_ZONE % 5 == 0: #zone mob nums
		method.bufferxy(37,15,"&RKTime: "+str(round(30.0-time.time()+BOSS_TIMER,1)) + "&XX") #boss timer
		if HIGHEST_ZONE > CURRENT_ZONE:
			method.bufferxy(45,3,"(1/1)")
		else:
			method.bufferxy(45,3,"(0/1)")
	else:
		if HIGHEST_ZONE > CURRENT_ZONE:
			method.bufferxy(45,3,"(10/10)")
		else:
			method.bufferxy(46,3,"("+str(ZONE_MOBS_KILLED)+"/10)")
	if MOB_HP <= 0: #mob death animation
		method.bufferxy(37,16,"rekt")
		if DEATH_FRAME == 20:
			DEATH_FRAME = 0
			createMob()
		else:
			for i in range(1,len(MOBS[CURRENT_MOB]) - (DEATH_FRAME/2)):
				method.bufferxy(33,(4 + (DEATH_FRAME/2)) +i,MOBS[CURRENT_MOB][i])
			DEATH_FRAME+=1
	else:
		for i in range(1,len(MOBS[CURRENT_MOB])): #mob drawing
			method.bufferxy(33,4+i,MOBS[CURRENT_MOB][i])
			method.bufferxy(37,16,method.dispBigNum(MOB_HP)) #mob hp
	if not quitMenuOpen:
		if lastch=='.': #characters at bottom
			method.bufferxy(37,24," < ")
