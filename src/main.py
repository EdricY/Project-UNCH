...
#Actually load
def load():
	global loadMenuOpen, newMenuOpen, returnMenuOpen, timeSinceLastSession
	#FOR NEW SAVEABLE VARIABLE, APPEND A VARIABLE TO THIS LIST
	...
	try:
		...
		if type(LD) == type("NEW"):
			...
			if typesMatch:
				...
				goldGained = 2 + timeSinceLastSession/(CURRENT_ZONE*CURRENT_ZONE+9+CURRENT_ZONE/2)*DPS*(CURRENT_ZONE*3 + 3*(1+CURRENT_ZONE/10))
				MONEY += goldGained
				returnMenuOpen=True
			else:
				loadMenuOpen = True
